...
def show_executive_dashboard(df):
    ...
    # Simpan copy data lengkap periode ini untuk hitung Lost Revenue
    ...
    # Hitung Lost Revenue ditarik dari seluruh pesanan non-sukses periode ini
    df_raw_unique = get_unique_orders(df_raw_period)
    success_statuses = (
        df_success["status_pesanan"].unique().tolist()
        if not df_success.empty
        else []
    )
    df_cancelled = df_raw_unique[
        ~df_raw_unique["status_pesanan"].isin(success_statuses)
    ]
    lost_revenue = df_cancelled["total_pembayaran"].fillna(0).sum()
